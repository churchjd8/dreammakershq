...
// --- Streamable HTTP transport (stateless, works on serverless) ---
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "1.0.0",
    status: "running",
    description: "Jeff Church MCP Server — voice profile, stories, contacts, and CPG Founders Group content",
    endpoints: {
      mcp: "/mcp (POST — Streamable HTTP, recommended)",
      sse: "/sse (GET — legacy SSE transport)"
    }
  });
});
...
app.listen(PORT, () => {
  console.log(`Jeff Church MCP server running on port ${PORT}`);
